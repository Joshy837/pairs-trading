...
def fetch_factor_prices(
    ...
    """
    Fetch pair + SPY + sector ETF prices for 3-factor analysis.

    Fetches lookback_days + 300 extra days so the 12-month momentum factor
    can be computed over the full analysis window (needs ~273 days of warmup).
    Returns (price1, price2, spy, sector) as aligned Series.
    """
    t1, t2, sector = ticker1.upper(), ticker2.upper(), sector_etf.upper()
    ...
    prices = prices[[t1, t2, "SPY", sector]].dropna()
